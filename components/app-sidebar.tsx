  ...
  return (
    <div className="space-y-1 mt-auto">
      {items.map((item) => {
        ...
        const isActive = pathname === item.url;
        return (
          <Link key={item.title} href={item.url} className={`flex items-center gap-2 px-3 py-2 rounded transition-colors duration-150 hover:bg-gray-100 dark:hover:bg-gray-800 ${isActive ? "bg-gray-200 dark:bg-gray-700 font-semibold" : ""}`}>
            <Icon className="!size-5" />
            <span className="text-sm">{item.title}</span>
          </Link>
        );
